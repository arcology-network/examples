...
async function main() {
    const rpcurl =nets[hre.network.name].url;
    const chainid =nets[hre.network.name].chainId;
    ...
    const txhash= await test_eth_sendRawTransaction(rpcurl,st,nets,provider,accounts);
...
async function test_eth_sendRawTransaction(url,contract,nets,provider,accounts) {
    try{
