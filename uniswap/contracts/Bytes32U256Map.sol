 ...
 */
contract Bytes32U256Map is Base { 
     ...
     */
    function insert(bytes32 key, uint256 value, uint256 lower, uint256 upper) public virtual{ 
     ...
     */
    function keyAt(uint256 idx) public virtual  returns(bytes32) {  
        bytes memory rawdata=Base.indToKey(idx);
        require(rawdata.length >= 32, "Input bytes must be at least 32 bytes long");
        bytes32 result;
        for (uint i = 0; i < 32; i++) {
            result |= bytes32(rawdata[i] & 0xFF) >> (i * 8);
        }
        return result;     
    }   
