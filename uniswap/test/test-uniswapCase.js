...
async function main() {
    ...
    const bt_factory = await ethers.getContractFactory("UniswapCase");
    const bt = await bt_factory.deploy();
    await bt.deployed();
    console.log(`Deployed UniswapCase Test at ${bt.address}`)
    ...
    /*
      ...
      for(i=1;i<=4;i++){
        txs.push(frontendUtil.generateTx(function([bt,from,val]){
          return bt.connect(from).add(from.address,val, {
            gasLimit: 500000000,
          });
        },bt,accounts[i],i));
      }
      for(i=1;i<=2;i=i+2){
        txs.push(frontendUtil.generateTx(function([bt,from,val]){
          return bt.connect(from).add(from.address,val*2, {
            gasLimit: 500000000,
          });
        },bt,accounts[i],i));
      }
   ...
   /*
      ...
      for(i=1;i<=4;i++){
        txs.push(frontendUtil.generateTx(function([bt,from]){
          return bt.connect(from).addbool(from.address,true, {
            gasLimit: 500000000,
          });
        },bt,accounts[i]));
      }
      for(i=1;i<=2;i=i+2){
        txs.push(frontendUtil.generateTx(function([bt,from]){
          return bt.connect(from).addbool(from.address,false, {
            gasLimit: 500000000,
          });
        },bt,accounts[i]));
      }
