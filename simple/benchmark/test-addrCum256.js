...
async function main() {
  ...
  const provider = new ethers.providers.JsonRpcProvider(nets[hre.network.name].url);
    ...
    const cum_factory = await ethers.getContractFactory("AddrCum256");
    ...
    if(createTxs){
      frontendUtil.ensurePath('data/adrcum256-add');
      const handle_add=frontendUtil.newFile('data/adrcum256-add/adrcum256-add.out')
      const counter=10;
      for (j=0;j<counter;j++) {
        for(i=0;i<accounts.length;i++){
          ...
          tx = await cum.connect(accounts[i]).populateTransaction.add(accounts[i].address,j);
